...
<template>
  <div class="tmsgBox" ref="tmsgBox">
    <div class="tmsg-respond" ref="respondBox">
      <h3>{{title}} <small v-show="isRespond" class="tcolorm" @click="removeRespond">取消回复</small></h3>
      <form class="">
        <el-input type="textarea" :rows="2" placeholder="还有什么问题吗？可以跟老师互动哦" v-model="textarea" @focus="pBody=true">
        </el-input>
        <div :class="pBody?'OwO':'OwO OwO-open'">
          <div class="OwO-logo" @click="pBody=!pBody" style="">
            <span>添加表情</span>
            <!-- <el-button type="primary" plain @click="submitOpinion">{{ask}}</el-button> -->
          </div>
          <div class="OwO-body">
        ...
        <!-- <el-row class="tmsg-r-info">
          <el-col :span="24" class="info-submit">
            <el-button type="primary" style="width:100%;" plain @click="submitOpinion">{{ask}}</el-button>
          </el-col>
        </el-row> -->
      </form>
    </div>
    <div class="tmsg-comments" ref="listDom">
      <!-- <a href="#" class="tmsg-comments-tip">总评论 {{total}} 条</a> -->
      <div class="tmsg-commentshow">
        <ul class="tmsg-commentlist">
          <li class="tmsg-c-item" v-for="(item,index) in msg" :key="'common'+index">
            <article class="">
              <header>
                <img :src="item.photo">
                <div class="i-name">
                  </span>{{item.replyName}}</span><span style="margin:0 5px;color: #238c8f;font-size:16px" v-show="item.replyName">回复</span><span>{{item.huifuOrPinglun}}</span>
                  <!-- {{item.huifuOrPinglun}} -->
                </div>
                <!-- <div class="i-class">
                  {{item.label}}
              ...
              <section>
                ...
                <div v-show="canReply" class="tmsg-replay" @click="respondMsg(item.id,item.huifuOrPinglun)">
                  回复
                </div>
              </section>
            </article>
            <ul v-show="1" class="tmsg-commentlist" style="padding-left:60px;">
              <li class="tmsg-c-item" v-for="(citem,cindex) in item.ChildsSon" :key="'citem'+cindex">
                <article class="">
                  ...
                  <section>
                    ...
                    <div v-show="haslogin" class="tmsg-replay" @click="respondMsg(item.id,item.huifuOrPinglun)">
                      回复
                    </div>
                  </section>
                </article>
  ...
  export default {
    ...
    methods: { //事件处理器
      ...
      respondMsg(id,huifuOrPinglun) { //回复留言
        ...
        this.title = "发表回复"
        this.isRespond = true;
        this.opinion.replyId = id;
        this.opinion.replyName = this.user.userName;
        this.opinion.huifuOrPinglun = huifuOrPinglun
            ...
            if(res.status == 200){
              if(res.data.user){
                ...
                if(this.phone == this.user.phone){
                  ...
                  this.canReply = true;
                  this.opinion.photo = this.user.photo;
                }
